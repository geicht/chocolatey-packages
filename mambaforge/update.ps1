...
function global:au_GetLatest {
    ...
    $jsonAnswer.assets | Where { $_.name -Match "^Mambaforge-$($jsonAnswer.tag_name)-Windows-x86_64.exe$" } | ForEach-Object {
...
function global:au_SearchReplace {
    @{
        ...
        ".\README.md" = @{
            "(\(https://community.chocolatey.org/packages/mambaforge)/.*?(\)$)" = "`${1}/$( $Latest.Version )`$2"
        }
    }
}
